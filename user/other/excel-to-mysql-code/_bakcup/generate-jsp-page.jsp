   ...
   <tr>
	 ...
	 <tr>
	      <td><span class="label">Update On ID Column</span></td>
				<td><input type="checkbox" name="UpdateOnIDColumn"  value="TRUE"/> Excel Col: <\%=HEADER[<%=IDColIndex %>] %></td></tr>
	 <tr>
...
// Main outer try catch block
    ...
    { 
		...
		boolean bUpdateOnIDCol = ( UpdateOnIDColumn!=null && "TRUE".equalsIgnoreCase(UpdateOnIDColumn))? true : false;
    <% 
		} 
		%>
						  ...
						  Iterator<String> itr_sql = col_list.iterator() ; 
							...
							{   
							   String st = itr_sql.next(); 
								 ...
								 String dbField = request.getParameter(st+".FieldName");
							          ...
							          while(itr_fld.hasNext())
												{
												   String st = itr_fld.next();
													 ...
													 int nSQLtype =  ((Integer)FldMap.get(st)).intValue();
													 switch(nSQLtype)
													 {
													 
													   case java.sql.Types.TINYINT:
														 %>byte bt<%=st %> = (byte)POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setByte( <%=n2+1 %> , bt<%=st %> ) ; 
															  <% 
															  if( bUpdateOnID ) { 
															  %> 
															  if( bUpdateOnIDCol )
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %>   stmtUpd.setByte( <%=n2+1 %> , bt<%=st %> ) ;  <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %>   stmtUpd.setByte( <%=WhrIdx %> , bt<%=st %> ) ;  stmtFind.setByte(1 , bt<%=st %> );        <% } %> 
															  <% 
																if( bUpdateOnID ) 
																{ 
																%> 
													   ...
													   case java.sql.Types.SMALLINT :
														 %>short n<%=st %> = (short)POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setShort( <%=n2+1 %> , n<%=st %> ) ; 
															  <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setShort( <%=n2+1 %> , n<%=st %> ) ;   <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %> stmtUpd.setShort( <%=WhrIdx %> , n<%=st %> ) ; stmtFind.setShort(1 , n<%=st %> ) ;  <% } %> 
														    <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>int n<%=st %> = (int)POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setInt( <%=n2+1 %> , n<%=st %> ) ; 
															  <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setInt( <%=n2+1 %> , n<%=st %> ) ;   <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %> stmtUpd.setInt( <%=WhrIdx %> , n<%=st %> ) ; stmtFind.setInt(1 , n<%=st %> ) ; <% } %> 
															  <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>long n<%=st %> = (long)POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setLong( <%=n2+1 %> , n<%=st %> ) ;
															<% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setLong( <%=n2+1 %> , n<%=st %> ) ;  <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %> stmtUpd.setLong( <%=WhrIdx %> , n<%=st %> ) ; stmtFind.setLong( 1 , n<%=st %> ) ;  <% } %> 
														    <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>float n<%=st %> = (float)POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setFloat( <%=n2+1 %> , n<%=st %> ) ; 
															 <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %>  stmtUpd.setFloat( <%=n2+1 %> , n<%=st %> ) ;   <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %>  stmtUpd.setFloat( <%=WhrIdx %> , n<%=st %> ) ; stmtFind.setFloat( 1 , n<%=st %> ) ;  <% } %> 
														   <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>double n<%=st %> = POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setDouble( <%=n2+1 %> , n<%=st %> ) ; 
															  <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setDouble( <%=n2+1 %> , n<%=st %> ) ;   <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %> stmtUpd.setDouble( <%=WhrIdx %> , n<%=st %> ) ; stmtFind.setDouble(1 , n<%=st %> ) ;  <% } %> 
	                              <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>BigDecimal bd<%=st %> = new BigDecimal( POIHelper.getNumberFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK)));
															 stmtIns.setBigDecimal( <%=n2+1 %> , bd<%=st %> ) ; 
															 <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setBigDecimal( <%=n2+1 %> , bd<%=st %> ) ;   <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %>  stmtUpd.setBigDecimal( <%=WhrIdx %> , bd<%=st %> ) ; stmtFind.setBigDecimal(1 , bd<%=st %> ) ; <% } %> 
														    <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>String str<%=st %> = POIHelper.getStringFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setString( <%=n2+1 %> , str<%=st %> ) ; 
															  <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %>  stmtUpd.setString( <%=n2+1 %> , str<%=st %> ) ;   <%  } %> 
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %>  stmtUpd.setString( <%=WhrIdx %> , str<%=st %> ) ; stmtFind.setString(1 , str<%=st %> ) ;  <% } %> 
														    <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>java.sql.Date dt<%=st %> = POIHelper.getDateFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setDate( <%=n2+1 %> , dt<%=st %> ) ; 
															 <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setDate( <%=n2+1 %> , dt<%=st %> ) ;   <%  } %>
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %> stmtUpd.setDate( <%=WhrIdx %> , dt<%=st %> ) ;  stmtFind.setDate(1 , dt<%=st %> ) ;   <% } %> 
														    <% 
																if( bUpdateOnID ) 
																{ 
																%> 
														 ...
														 %>java.sql.Time tm<%=st %> = POIHelper.getDateFromCell(datarow.getCell(<%=n2 %>, Row.CREATE_NULL_AS_BLANK));
															 stmtIns.setTime( <%=n2+1 %> , tm<%=st %> ) ;
															 <% 
																if( bUpdateOnID ) 
																{ 
																%> 
															  ...
															  <%  
																...
																%>
															 <% if( bUpdateOnID ) { %> stmtUpd.setTime( <%=n2+1 %> , tm<%=st %> ) ;  <%  } %>  
															 <% if( bUpdateOnID == true && n2 == IDColIndex ) { %> stmtUpd.setTime( <%=WhrIdx %> , tm<%=st %> ) ; stmtFind.setTime( 1 , tm<%=st %> ) ;  <% } %> 
														   <% 
																if( bUpdateOnID ) 
																{ 
																%> 
