...
response.sendRedirect(response.encodeRedirectURL(thisFile+"?Action=Show&AdminID="+nAdminID));
...
<button onclick="NavigateTo('<%=thisFile %>?Action=Show&AdminID=<%=SiMngrBn.AdminID %><%=ForeignKeyParam %><%=ReturnPathLink %>')" class="btn btn-pure btn-primary icon wb-eye" type="button" style="font-size: 20px;padding: 0;margin-left: 5px;" data-original-title="View" data-trigger="hover" data-placement="top" data-toggle="tooltip"></button>
	...
	<td><%if(SiMngrBn.Gender.equalsIgnoreCase("Male")){ %><i aria-hidden="true" class="icon fa fa-male iccolor"></i><% }else{ %><i class="icon fa fa-female iccolor" aria-hidden="true"></i><% } %>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="callselect('<%=SiMngrBn.AdminID %>','<%=ShowItem.showAdminName(SiMngrBn.AdminID, 1) %>')"><%=ShowItem.showAdminName(SiMngrBn.AdminID, 1) %></a></td>
