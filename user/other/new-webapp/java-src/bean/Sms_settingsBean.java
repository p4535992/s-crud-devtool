...
/*
   ...
   The JDBC driver version is: mysql-connector-java-5.1.38 ( Revision: fe541c166cec739c74cc727c5da96c1028b4834a )
	 Autonumber column is: SmsSettingID

*/
public class Sms_settingsBean
{
	
/* JNDI data source name of database */
	public String _datasource = "jdbc/$DATABASE" ;
/* Table name in SQL databse */
	public String _tablename = "sms_settings"; 
...
public void copyTo(com.db.$DATABASE.Sms_settingsBean other )
{
   other.SmsSettingID = this.SmsSettingID ;
...
public void copyFrom(com.db.$DATABASE.Sms_settingsBean other )
{
   this.SmsSettingID = other.SmsSettingID ;
...
throws java.sql.SQLException 
{
stmt.setInt( 1 , SmsSettingID ) ; 
stmt.setShort( 2 , Type ) ; 
stmt.setString( 3 , Name ) ; 
stmt.setString( 4 , Description ) ; 
stmt.setString( 5 , Value ) ; 
stmt.setString( 6 , URL ) ; 
stmt.setTimestamp( 7 , UpdateDateTime ) ; 
...
throws java.sql.SQLException
{
SmsSettingID = rslt.getInt("SmsSettingID"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `sms_settings` ( `SmsSettingID`, `Type`, `Name`, `Description`, `Value`, `URL`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) SmsSettingID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `sms_settings` ( `SmsSettingID`, `Type`, `Name`, `Description`, `Value`, `URL`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?, ?, ?  ) ";
...
public boolean updateRecord(int update_SmsSettingID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `sms_settings` SET `SmsSettingID` = ?, `Type` = ?, `Name` = ?, `Description` = ?, `Value` = ?, `URL` = ?, `UpdateDateTime` = ?  WHERE `sms_settings`.`SmsSettingID` = "+update_SmsSettingID+" " ;
...
public boolean deleteRecord(int delete_SmsSettingID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `sms_settings` WHERE `sms_settings`.`SmsSettingID` = "+delete_SmsSettingID+" " ;
...
public boolean  locateRecord(int locate_SmsSettingID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `sms_settings` WHERE  `sms_settings`.`SmsSettingID` = "+locate_SmsSettingID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `sms_settings`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `sms_settings` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `sms_settings`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `sms_settings`  "+Join+" `"+JoinTableName+"` ON (`sms_settings`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `sms_settings`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `sms_settings`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `sms_settings`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `sms_settings`  "+Join+" `"+JoinTableName+"` ON (`sms_settings`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `sms_settings`  "+Join+" `"+JoinTableName+"` ON (`sms_settings`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for Sms_settingsBean */	
