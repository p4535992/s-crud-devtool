...
import  javax.sql.* ;	
/*
   Code generated on : 17/11/2016 10:08 PM
   ...
   The JDBC driver version is: mysql-connector-java-5.1.38 ( Revision: fe541c166cec739c74cc727c5da96c1028b4834a )
	 Autonumber column is: TemplateID

*/
public class Sms_templateBean
{
	
/* JNDI data source name of database */
	public String _datasource = "jdbc/$DATABASE" ;
/* Table name in SQL databse */
	public String _tablename = "sms_template"; 
...
public void copyTo(com.db.$DATABASE.Sms_templateBean other )
{
...
public void copyFrom(com.db.$DATABASE.Sms_templateBean other )
{
...
throws java.sql.SQLException
{
TemplateID = rslt.getInt("TemplateID"); 
Name = rslt.getString("Name"); 
Text = rslt.getString("Text"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `sms_template` ( `TemplateID`, `Name`, `Text` ) VALUES ( ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) TemplateID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `sms_template` ( `TemplateID`, `Name`, `Text` ) VALUES ( ?, ?, ?  ) ";
...
public boolean updateRecord(int update_TemplateID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `sms_template` SET `TemplateID` = ?, `Name` = ?, `Text` = ?  WHERE `sms_template`.`TemplateID` = "+update_TemplateID+" " ;
...
public boolean deleteRecord(int delete_TemplateID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `sms_template` WHERE `sms_template`.`TemplateID` = "+delete_TemplateID+" " ;
...
public boolean  locateRecord(int locate_TemplateID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `sms_template` WHERE  `sms_template`.`TemplateID` = "+locate_TemplateID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `sms_template`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `sms_template` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `sms_template`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `sms_template`  "+Join+" `"+JoinTableName+"` ON (`sms_template`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `sms_template`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `sms_template`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `sms_template`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `sms_template`  "+Join+" `"+JoinTableName+"` ON (`sms_template`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `sms_template`  "+Join+" `"+JoinTableName+"` ON (`sms_template`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for Sms_templateBean */	
