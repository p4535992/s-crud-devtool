...
import  javax.sql.* ;	
/*
   Code generated on : 15/11/2016 09:39 AM
...
/* Variables corrosponding to table fields - field count: 5 */
	 
    public int StateCityID = 0 ;  
    public String City = "" ;  
    public String State = "" ;  
...
public void copyTo(com.db.$DATABASE.State_cityBean other )
{
   other.StateCityID = this.StateCityID ;
...
public void copyFrom(com.db.$DATABASE.State_cityBean other )
{
   this.StateCityID = other.StateCityID ;
...
throws java.sql.SQLException 
{
stmt.setInt( 1 , StateCityID ) ; 
stmt.setString( 2 , City ) ; 
stmt.setString( 3 , State ) ; 
stmt.setString( 4 , Country ) ; 
stmt.setTimestamp( 5 , UpdateDateTime ) ; 
...
throws java.sql.SQLException
{
StateCityID = rslt.getInt("StateCityID"); 
City = rslt.getString("City"); 
State = rslt.getString("State"); 
Country = rslt.getString("Country"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `state_city` ( `StateCityID`, `City`, `State`, `Country`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) StateCityID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `state_city` ( `StateCityID`, `City`, `State`, `Country`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?  ) ";
...
public boolean updateRecord(int update_StateCityID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `state_city` SET `StateCityID` = ?, `City` = ?, `State` = ?, `Country` = ?, `UpdateDateTime` = ?  WHERE `state_city`.`StateCityID` = "+update_StateCityID+" " ;
...
public boolean deleteRecord(int delete_StateCityID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `state_city` WHERE `state_city`.`StateCityID` = "+delete_StateCityID+" " ;
...
public boolean  locateRecord(int locate_StateCityID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `state_city` WHERE  `state_city`.`StateCityID` = "+locate_StateCityID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `state_city`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `state_city` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `state_city`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `state_city`  "+Join+" `"+JoinTableName+"` ON (`state_city`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `state_city`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `state_city`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `state_city`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `state_city`  "+Join+" `"+JoinTableName+"` ON (`state_city`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `state_city`  "+Join+" `"+JoinTableName+"` ON (`state_city`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for State_cityBean */	
