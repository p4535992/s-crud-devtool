...
import  javax.sql.* ;	
/*
   Code generated on : 12/08/2016 05:09 PM
...
/* Variables corrosponding to table fields - field count: 8 */
	...
	public java.sql.Date BackupDate ; 
...
public void copyTo(com.db.$DATABASE.Daily_backupBean other )
{
   other.BackupID = this.BackupID ;
   other.Path = this.Path ;
   other.FileName = this.FileName ;
   other.PathDate = this.PathDate ;
...
public void copyFrom(com.db.$DATABASE.Daily_backupBean other )
{
   this.BackupID = other.BackupID ;
   this.Path = other.Path ;
   this.FileName = other.FileName ;
   this.PathDate = other.PathDate ;
   this.BackupDate = other.BackupDate ;
...
stmt.setString( 4 , PathDate ) ; 
stmt.setDate( 5 , BackupDate ) ; 
stmt.setTimestamp( 6 , DateTime ) ; 
stmt.setShort( 7 , Flag ) ; 
stmt.setTimestamp( 8 , UpdateDateTime ) ; 
...
throws java.sql.SQLException
{
BackupID = rslt.getInt("BackupID"); 
Path = rslt.getString("Path"); 
FileName = rslt.getString("FileName"); 
PathDate = rslt.getString("PathDate"); 
BackupDate = rslt.getDate("BackupDate"); 
DateTime = rslt.getTimestamp("DateTime"); 
Flag = rslt.getShort("Flag"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `daily_backup` ( `BackupID`, `Path`, `FileName`, `PathDate`, `BackupDate`, `DateTime`, `Flag`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) BackupID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `daily_backup` ( `BackupID`, `Path`, `FileName`, `PathDate`, `BackupDate`, `DateTime`, `Flag`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?  ) ";
...
public boolean updateRecord(int update_BackupID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `daily_backup` SET `BackupID` = ?, `Path` = ?, `FileName` = ?, `PathDate` = ?, `BackupDate` = ?, `DateTime` = ?, `Flag` = ?, `UpdateDateTime` = ?  WHERE `daily_backup`.`BackupID` = "+update_BackupID+" " ;
...
public boolean deleteRecord(int delete_BackupID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `daily_backup` WHERE `daily_backup`.`BackupID` = "+delete_BackupID+" " ;
...
public boolean  locateRecord(int locate_BackupID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `daily_backup` WHERE  `daily_backup`.`BackupID` = "+locate_BackupID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `daily_backup`  WHERE `"+field+"` = '"+value+"' " ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `daily_backup`  "+WhereClause+" "+OrderByClause ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `daily_backup`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 java.sql.Connection conn = Connect();
 if(conn==null) return 0;
 String query_count = "  SELECT COUNT(*) FROM `daily_backup`  "+WhereClause;
    try
    {
...
} /* End of class declaration for Daily_backupBean */	
