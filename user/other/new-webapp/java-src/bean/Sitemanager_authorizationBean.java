...
import  javax.sql.* ;	
/*
   Code generated on : 14/11/2016 11:05 PM
   ...
   The JDBC driver version is: mysql-connector-java-5.1.38 ( Revision: fe541c166cec739c74cc727c5da96c1028b4834a )
	 Autonumber column is: AuthorizationID

*/
public class Sitemanager_authorizationBean
{
	
/* JNDI data source name of database */
	public String _datasource = "jdbc/$DATABASE" ;
/* Table name in SQL databse */
	public String _tablename = "sitemanager_authorization"; 
...
/* Variables corrosponding to table fields - field count: 6 */
    ...
    public int AuthorizationID = 0 ;  
    public int AdminID = 0 ;  
    public int ModuleID = 0 ;  
    public String ModuleName = "" ;  
    public String ModuleActivityID = "" ;  
...
public void copyTo(com.db.$DATABASE.Sitemanager_authorizationBean other )
{
   other.AuthorizationID = this.AuthorizationID ;
   other.AdminID = this.AdminID ;
   other.ModuleID = this.ModuleID ;
   other.ModuleName = this.ModuleName ;
   other.ModuleActivityID = this.ModuleActivityID ;
...
public void copyFrom(com.db.$DATABASE.Sitemanager_authorizationBean other )
{
   this.AuthorizationID = other.AuthorizationID ;
   this.AdminID = other.AdminID ;
   this.ModuleID = other.ModuleID ;
   this.ModuleName = other.ModuleName ;
   this.ModuleActivityID = other.ModuleActivityID ;
...
throws java.sql.SQLException 
{
stmt.setInt( 1 , AuthorizationID ) ; 
stmt.setInt( 2 , AdminID ) ; 
stmt.setInt( 3 , ModuleID ) ; 
stmt.setString( 4 , ModuleName ) ; 
stmt.setString( 5 , ModuleActivityID ) ; 
stmt.setTimestamp( 6 , UpdateDateTime ) ; 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `sitemanager_authorization` ( `AuthorizationID`, `AdminID`, `ModuleID`, `ModuleName`, `ModuleActivityID`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) AuthorizationID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `sitemanager_authorization` ( `AuthorizationID`, `AdminID`, `ModuleID`, `ModuleName`, `ModuleActivityID`, `UpdateDateTime` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
...
public boolean updateRecord(int update_AuthorizationID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `sitemanager_authorization` SET `AuthorizationID` = ?, `AdminID` = ?, `ModuleID` = ?, `ModuleName` = ?, `ModuleActivityID` = ?, `UpdateDateTime` = ?  WHERE `sitemanager_authorization`.`AuthorizationID` = "+update_AuthorizationID+" " ;
...
public boolean deleteRecord(int delete_AuthorizationID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `sitemanager_authorization` WHERE `sitemanager_authorization`.`AuthorizationID` = "+delete_AuthorizationID+" " ;
...
public boolean  locateRecord(int locate_AuthorizationID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `sitemanager_authorization` WHERE  `sitemanager_authorization`.`AuthorizationID` = "+locate_AuthorizationID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `sitemanager_authorization`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `sitemanager_authorization` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `sitemanager_authorization`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `sitemanager_authorization`  "+Join+" `"+JoinTableName+"` ON (`sitemanager_authorization`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `sitemanager_authorization`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `sitemanager_authorization`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `sitemanager_authorization`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `sitemanager_authorization`  "+Join+" `"+JoinTableName+"` ON (`sitemanager_authorization`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `sitemanager_authorization`  "+Join+" `"+JoinTableName+"` ON (`sitemanager_authorization`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for Sitemanager_authorizationBean */	
