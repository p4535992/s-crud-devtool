...
/*
   ...
   The JDBC driver version is: mysql-connector-java-5.1.38 ( Revision: fe541c166cec739c74cc727c5da96c1028b4834a )
	 Autonumber column is: MailDebugLogID

*/
public class Mail_debuglogBean
{
	
/* JNDI data source name of database */
	public String _datasource = "jdbc/$DATABASE" ;
/* Table name in SQL databse */
	public String _tablename = "mail_debuglog"; 
...
public void copyTo(com.db.$DATABASE.Mail_debuglogBean other )
{
   other.MailDebugLogID = this.MailDebugLogID ;
...
public void copyFrom(com.db.$DATABASE.Mail_debuglogBean other )
{
   this.MailDebugLogID = other.MailDebugLogID ;
...
throws java.sql.SQLException 
{
stmt.setInt( 1 , MailDebugLogID ) ; 
stmt.setTimestamp( 2 , LogDateTime ) ; 
stmt.setString( 3 , MailTo ) ; 
...
throws java.sql.SQLException
{
MailDebugLogID = rslt.getInt("MailDebugLogID"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `mail_debuglog` ( `MailDebugLogID`, `LogDateTime`, `MailTo`, `Subject`, `Context`, `LogText` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) MailDebugLogID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `mail_debuglog` ( `MailDebugLogID`, `LogDateTime`, `MailTo`, `Subject`, `Context`, `LogText` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
...
public boolean updateRecord(int update_MailDebugLogID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `mail_debuglog` SET `MailDebugLogID` = ?, `LogDateTime` = ?, `MailTo` = ?, `Subject` = ?, `Context` = ?, `LogText` = ?  WHERE `mail_debuglog`.`MailDebugLogID` = "+update_MailDebugLogID+" " ;
...
public boolean deleteRecord(int delete_MailDebugLogID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `mail_debuglog` WHERE `mail_debuglog`.`MailDebugLogID` = "+delete_MailDebugLogID+" " ;
...
public boolean  locateRecord(int locate_MailDebugLogID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `mail_debuglog` WHERE  `mail_debuglog`.`MailDebugLogID` = "+locate_MailDebugLogID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `mail_debuglog`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `mail_debuglog` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `mail_debuglog`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `mail_debuglog`  "+Join+" `"+JoinTableName+"` ON (`mail_debuglog`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `mail_debuglog`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `mail_debuglog`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `mail_debuglog`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `mail_debuglog`  "+Join+" `"+JoinTableName+"` ON (`mail_debuglog`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `mail_debuglog`  "+Join+" `"+JoinTableName+"` ON (`mail_debuglog`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for Mail_debuglogBean */	
