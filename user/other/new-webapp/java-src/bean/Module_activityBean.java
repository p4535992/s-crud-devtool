...
/*
   ...
   The JDBC driver version is: mysql-connector-java-5.1.38 ( Revision: fe541c166cec739c74cc727c5da96c1028b4834a )
	 Autonumber column is: ModuleActivityID

*/
public class Module_activityBean
{
	
/* JNDI data source name of database */
	public String _datasource = "jdbc/$DATABASE" ;
/* Table name in SQL databse */
	public String _tablename = "module_activity"; 
...
/* Variables corrosponding to table fields - field count: 3 */
    ...
    public int ModuleActivityID = 0 ;  
    public String ModuleActivityName = "" ;  
...
public void copyTo(com.db.$DATABASE.Module_activityBean other )
{
   other.ModuleActivityID = this.ModuleActivityID ;
   other.ModuleActivityName = this.ModuleActivityName ;
...
public void copyFrom(com.db.$DATABASE.Module_activityBean other )
{
   this.ModuleActivityID = other.ModuleActivityID ;
   this.ModuleActivityName = other.ModuleActivityName ;
...
{
	this.ModuleActivityID = 0 ; 
	this.ModuleActivityName = "" ; 
...
throws java.sql.SQLException 
{
stmt.setInt( 1 , ModuleActivityID ) ; 
stmt.setString( 2 , ModuleActivityName ) ; 
...
throws java.sql.SQLException
{
ModuleActivityID = rslt.getInt("ModuleActivityID"); 
ModuleActivityName = rslt.getString("ModuleActivityName"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `module_activity` ( `ModuleActivityID`, `ModuleActivityName`, `UpdateDateTime` ) VALUES ( ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) ModuleActivityID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `module_activity` ( `ModuleActivityID`, `ModuleActivityName`, `UpdateDateTime` ) VALUES ( ?, ?, ?  ) ";
...
public boolean updateRecord(int update_ModuleActivityID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `module_activity` SET `ModuleActivityID` = ?, `ModuleActivityName` = ?, `UpdateDateTime` = ?  WHERE `module_activity`.`ModuleActivityID` = "+update_ModuleActivityID+" " ;
...
public boolean deleteRecord(int delete_ModuleActivityID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `module_activity` WHERE `module_activity`.`ModuleActivityID` = "+delete_ModuleActivityID+" " ;
...
public boolean  locateRecord(int locate_ModuleActivityID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `module_activity` WHERE  `module_activity`.`ModuleActivityID` = "+locate_ModuleActivityID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `module_activity`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `module_activity` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `module_activity`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `module_activity`  "+Join+" `"+JoinTableName+"` ON (`module_activity`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `module_activity`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `module_activity`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `module_activity`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `module_activity`  "+Join+" `"+JoinTableName+"` ON (`module_activity`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `module_activity`  "+Join+" `"+JoinTableName+"` ON (`module_activity`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for Module_activityBean */	
