...
import  javax.sql.* ;	
/*
   Code generated on : 31/05/2016 11:25 PM
...
public void copyTo(com.db.$DATABASE.DebuglogBean other )
{
...
public void copyFrom(com.db.$DATABASE.DebuglogBean other )
{
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `debuglog` ( `LogID`, `LogTime`, `Context`, `LogText` ) VALUES ( ?, ?, ?, ?  ) ";
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `debuglog` ( `LogID`, `LogTime`, `Context`, `LogText` ) VALUES ( ?, ?, ?, ?  ) ";
	...
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `debuglog` SET `LogID` = ?, `LogTime` = ?, `Context` = ?, `LogText` = ?  WHERE `debuglog`.`LogID` = "+update_LogID+" " ;
...
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `debuglog` WHERE `debuglog`.`LogID` = "+delete_LogID+" " ;
...
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `debuglog` WHERE  `debuglog`.`LogID` = "+locate_LogID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `debuglog`  WHERE `"+field+"` = '"+value+"' " ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `debuglog`  "+WhereClause+" "+OrderByClause ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `debuglog`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 java.sql.Connection conn = Connect();
 if(conn==null) return 0;
 String query_count = "  SELECT COUNT(*) FROM `debuglog`  "+WhereClause;
    try
    {
