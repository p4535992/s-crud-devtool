...
/*
   ...
   The JDBC driver version is: mysql-connector-java-5.1.38 ( Revision: fe541c166cec739c74cc727c5da96c1028b4834a )
	 Autonumber column is: MailSettingID

*/
public class Mail_settingsBean
{
	
/* JNDI data source name of database */
	public String _datasource = "jdbc/$DATABASE" ;
/* Table name in SQL databse */
	public String _tablename = "mail_settings"; 
...
public void copyTo(com.db.$DATABASE.Mail_settingsBean other )
{
   other.MailSettingID = this.MailSettingID ;
...
public void copyFrom(com.db.$DATABASE.Mail_settingsBean other )
{
   this.MailSettingID = other.MailSettingID ;
...
throws java.sql.SQLException
{
MailSettingID = rslt.getInt("MailSettingID"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `mail_settings` ( `MailSettingID`, `Type`, `Name`, `Description`, `Value`, `URL` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
     ...
     */
     if ( _autonumber > 0 ) MailSettingID = _autonumber ;
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `mail_settings` ( `MailSettingID`, `Type`, `Name`, `Description`, `Value`, `URL` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
...
public boolean updateRecord(int update_MailSettingID)
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `mail_settings` SET `MailSettingID` = ?, `Type` = ?, `Name` = ?, `Description` = ?, `Value` = ?, `URL` = ?  WHERE `mail_settings`.`MailSettingID` = "+update_MailSettingID+" " ;
...
public boolean deleteRecord(int delete_MailSettingID)
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `mail_settings` WHERE `mail_settings`.`MailSettingID` = "+delete_MailSettingID+" " ;
...
public boolean  locateRecord(int locate_MailSettingID )
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `mail_settings` WHERE  `mail_settings`.`MailSettingID` = "+locate_MailSettingID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `mail_settings`  WHERE `"+field+"` = '"+value+"' " ;
...
throws java.sql.SQLException 
{
   ...
   try
	{
		StringBuilder sbQry  = new StringBuilder( " SELECT * FROM `mail_settings` WHERE ") ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `mail_settings`  "+WhereClause+" "+OrderByClause ;
...
/* General purpose Join table opening method  */
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	    if(_conn==null) return null;
      _query =   "  SELECT * FROM `mail_settings`  "+Join+" `"+JoinTableName+"` ON (`mail_settings`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`) "+WhereClause+" "+OrderByClause ;
      _stmt = _conn.createStatement();
      _result  = _stmt.executeQuery(_query); 
      return _result ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `mail_settings`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `mail_settings`  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `mail_settings`  "+WhereClause;	 
 }	 
    try
    {
...
throws java.sql.SQLException 
{
 ...
 if(WhereClause.contains("GROUP BY"))
 {
	query_count = "SELECT COUNT(*) FROM ( SELECT COUNT(*) FROM `mail_settings`  "+Join+" `"+JoinTableName+"` ON (`mail_settings`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause+") AS X ";
 }
 else
 {
	query_count = " SELECT COUNT(*) FROM `mail_settings`  "+Join+" `"+JoinTableName+"` ON (`mail_settings`.`"+OnIDField+"` = `"+JoinTableName+"`.`"+OnIDField+"`)  "+WhereClause;	 
 }	 
    try
    {
    java.sql.Statement stmt_count = conn.createStatement();
...
} /* End of class declaration for Mail_settingsBean */	
