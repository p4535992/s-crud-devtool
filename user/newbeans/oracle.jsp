...
// ( for debugging ) response.setContentType("text/plain");

/*  
    Oracle specific code
    Oracle does not support detction of auto incrementing primary keys
    through DatabaseMetaData method isAutoincrement();
    Threfore Autoincrement column has to be obtained from request only
*/
String IsAuto = request.getParameter("IsAuto") ;
boolean bIsAuto = (IsAuto!=null) ? true : false ;
...
//  col.Auto = rsmd.isAutoIncrement(n); // Not applicable to oracle :
// Do guesswork from request parameters
if( IsAuto != null && bIntegerIDField ==true && col.ColName.equalsIgnoreCase(IDField) )
{
   col.Auto = true ;
}
else
...
switch(col.ColSQLType)
	{
      ...
      break ;
    	...
    	case java.sql.Types.NUMERIC:
			    // Oracle is funny ! converts all INTEGER to NUMERIC
					// Do wild guesswork to find data types
					// Decimal number have scales > 0 in oracle and integers have 0 scale
			    if(col.Scale > 0 )
					{ 
	  	       col.GetMethod = "getBigDecimal(\""+col.ColName+"\")";
	  	       col.SetMethod = "setBigDecimal( "+(n-nAuto)+" , "+col.VarName+" ) ";
...
The code is  customized for Oracle JDBC.
The driver is:   <%=DriverName %> driver.
The JDBC driver version is: <%=md.getDriverVersion() %>
Autonumber column is: <%=AutoColVariable %> ( Guesswork in case of oracle )
*/
...
/* Variables corrosponding to table fields - field count: <%=columns.size() %> */
 ...
 <%
	...
	%><%  if( bIsAuto ){ %><jsp:include page="oracle_hack.jsp" flush="true"><jsp:param name="AddSQL" value="<%=AddSQL %>" /><jsp:param name="count" value="<%=count %>" /></jsp:include><% }	 %>
...
throws java.sql.SQLException 
{
 ...
 catch(javax.naming.NamingException ex)
 {
  ...
  try
   ...
   conx = DriverManager.getConnection ("jdbc:oracle:thin:@127.0.0.1:1521:<%=JNDIDSN.substring("jdbc/".length() ) %>", "root", "");
  }
...
throws java.sql.SQLException 
{
  <% 
	...
	{ 
	%>
    if(_oracle_hack == true) return addOracleRecord() ;
  <% 
	} %>
...
boolean  bAdded = false  ;	
 ...
 try{
    String query_add = " <%=AddSQL.toString() %> ";
		...
		%>
		/* 
		   Oracle requires special handling for auto generated keys.
		   Pass String array of column names used for making autoinc. key
			 as second parameter to prepareStatment method.
		*/
		 String[] AutoColumnNames = { "<%=IDField %>" } ;
		 java.sql.PreparedStatement stmt_add = conn.prepareStatement(query_add, AutoColumnNames ) ; // Statement.RETURN_GENERATED_KEYS not applicable
		<% 
		}
		else
		{
		 %>
		java.sql.PreparedStatement stmt_add = conn.prepareStatement(query_add) ; // Statement.RETURN_GENERATED_KEYS not applicable
		<% 
		} 
		%>
		//
    ...
    try
       ...
       if (rs_auto.next()) _autonumber = rs_auto.getInt(1);
		...
		{
		%>_autonumber=0; // There is No AUTOINCREMENT field found in this tabel<% 
		} // end if if(nAuto > 0 ) 
		%>
		stmt_add.close();
    }
 ...
 throws java.sql.SQLException 
{
  <% 
	...
	{ 
	%>
    if(_oracle_hack == true) return beginInsertOracle() ;
  <% 
	} 
	%>
   ...
   String query_insert = " <%=AddSQL.toString() %> ";
		...
		*/
		 String[] AutoColumnNames = { "<%=IDField %>" } ;
		 _stmt_insert = _conn_insert.prepareStatement(query_insert, AutoColumnNames ) ; // Statement.RETURN_GENERATED_KEYS not applicable
		<% 
		}
		else
		{
		 %>
		 _stmt_insert  = _conn_insert.prepareStatement(query_insert) ; // Statement.RETURN_GENERATED_KEYS not applicable
		<% 
		} 
		%>
 ...
 throws java.sql.SQLException 
{
	...
	{ 
	%>
    if(_oracle_hack == true) return continueInsertOracle();
  <% 
	} 
	%> 
  ...
  if( _stmt_insert.executeUpdate() == 1) bInsDone = true ;
   ...
   rs_auto_ins.close();
	 ...
	 { 
	 %>_autonumber=0;  // No AUTOINCREMENT Field found in table<% 
	 } // end   if(nAuto > 0  
	 %>
		<% if(AutoColVariable!=null)
			 {
