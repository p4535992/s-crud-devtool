...
The detected driver is:   <%=DriverName %> driver.
...
boolean  bAdded = false  ;	
 ...
 try{
    String query_add = psql.SQL(" <%=AddSQL.toString() %> " );
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = psql.SQL(" <%=AddSQL.toString() %> ");
...
%>
  ...
  try
	  {
    String query_update = psql.SQL(" <%=UpdSQL %><%=Quotes %>"+update_<%=IDField %>+"<%=Quotes %> ") ;
...
%>
 ...
 if(conn==null) return false ;
 try{
    String query_del = psql.SQL(" <%=DeleteSQL %><%=Quotes %>"+delete_<%=IDField %>+"<%=Quotes %> ") ;
...
%>
 ...
 if(conn==null) return false ;
 try{
    String query_locate = psql.SQL(" <%=LocateSQL %><%=Quotes %>"+locate_<%=IDField %>+"<%=Quotes %> " ) ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field =  psql.SQL(" <%=OpenTableSQL  %> WHERE `"+field+"` = '"+value+"' ") ;
...
{
  _conn = Connect();
  _query =   psql.SQL(" <%=OpenTableSQL %> "+WhereClause+" "+OrderByClause) ;
...
throws java.sql.SQLException
{
 ...
 java.sql.Connection conn = Connect();
 if(conn==null) return 0;
 String query_count = psql.SQL(" <%=RecordCountSQL %> "+WhereClause );
    try
    {
