...
$BeanName.executeUpdate("UPDATE `$TABLENAME` SET `Password`='"+ResetPassword+"' "+WhereClause+" ");
    ...
    qStr  = new com.webapp.utils.MakeQueryString( request, application );
	  ...
	  String ChkQuery = qStr.SQL( " SELECT * FROM  `$TABLENAME` "+ WhrCls + " ORDER BY `$IDField` " );
		...
		if("ClearStatus".equalsIgnoreCase(CheckedAction) && chk_count > 0  )
		{
				 ...
				 int st_cnt = $BeanName.executeUpdate( qStr.SQL( "UPDATE `$TABLENAME` SET `LoginStatus`="+LoginStatusFlag.NOT_LOGGED+" "+WhrCls+" ") );
...
<head>
  <jsp:include page ="/include-page/common/meta-tag.jsp" flush="true" />
	<title>Reset $CTABLENAME Password</title>
...
<body class="<%=menuTypeClass %> <%=SiMngrBn.LoginRole %>" >
  ...
  <div class="page animsition">
    <div class="page-content container-fluid">
  	  <div class="row">
        <div class="col-sm-6">
  			  <h3 class="page-title-res"><i aria-hidden="true" class="icon fa fa-cogs iccolor"></i>Reset $CTABLENAME Password</h3>
  	    </div>
        <div class="col-sm-6">
          <ol class="breadcrumb breadcrumb-res">
            ...
            <li class="active">Reset $CTABLENAME Password</li>
          </ol>			
  	    </div>
  		</div>	
			<hr class="hr-res" />
...
<button onclick="NavigateTo('<%=thisFile %>?Action=Show&$IDField=<%=$BeanName.$IDField %><%=ForeignKeyParam %><%=ReturnPathLink %>')" class="btn btn-pure btn-primary icon wb-eye" type="button" style="font-size: 20px;padding: 0;margin-left: 5px;" data-original-title="View" data-trigger="hover" data-placement="top" data-toggle="tooltip"></button>
	...
	<td><a href="javascript:void(0)" onclick="ResetPassword('<%=$BeanName.$IDField %>')">Reset Password</a></td>
	<td>
	<% if($BeanName.LoginStatus == LoginStatusFlag.LOGGED){ %>
	<a href="javascript:void(0)" onclick="AppConfirm('question','Are you sure ?','<%=thisFile %>?Action=ClearSession&$IDField=<%=$BeanName.$IDField %><%=ForeignKeyParam %><%=ReturnPathLink %>','Clear Current Login Status of \n<%=ShowItem.show$CTABLENAMEName($BeanName.$IDField, 1) %> ?')">Clear <!-- Current Login  -->Status</a>
	<% }else{ %>
	NOT LOGGED
	<% } %>
	</td>
