...
<body class="<%=SiMngrBn.LoginRole %>" style="padding-top: 0px;" >
...
%>			
<form action="<%=thisFile %>" method="POST"  accept-charset="UTF-8" name="$TABLENAME_Add" id="$TABLENAME_Add"   >
...
$BeanName.locateRecord(n$IDField);
%>
<form action="<%=thisFile %>" method="POST"  accept-charset="UTF-8" name="$TABLENAME_Update" id="$TABLENAME_Update"    >
...
%>
<form action="<%=thisFile %>" method="POST"  accept-charset="UTF-8" class="form-horizontal" id="$TABLENAME_Search">
...
<div class="panel-body panel-form-box-body container-fluid">
      <div class="form-group">
        <label class="control-label blue-grey-600 col-sm-3">Search By ID :
      	...
      	<div class="col-sm-1" id="$IDField_OPR_BLK"><%=NumberSearchdroplist.getOperatorList("$IDField_OPERATOR", "$IDField_OPERATOR", "form-control show-tick", "data-plugin='selectpicker'" )  %></div>
      	<span class="visible-xs-*">&nbsp;</span>
      	<div class="col-sm-2"><input type="text" name="$IDField" id="$IDField" class="form-control" /></div>
      	<label class="control-label blue-grey-600 col-sm-1 text-center" id="$IDField_RNG_LBL" style="display:none;width: 5.333%;margin-bottom: 0px;">TO</label>
      	<div class="col-sm-2" id="$IDField_RNG_BLK" style="display:none"><input type="text" name="$IDField_TO" id="$IDField_TO" class="form-control" /></div>
      </div>			
      ...
      <div class="form-group">
        ...
        <div class="col-sm-2"><%=CurrentStatusFlag.getDropList("CurrentStatus", "CurrentStatus", CurrentStatusFlag.OK, true, false, "form-control show-tick", "data-plugin='selectpicker' data-container='body' data-live-search='false'") %></div>
      </div>	
...
else
{
	...
	else // else of  if(QryObj != null )
	{
     ...
     // qStr.setOrderByClause(" ORDERBY ? ");
	   ...
	   if(request.getParameter("$IDField_RNG_CHK") != null)
		 {
		    qStr.addNumberInRangeParam("$IDField", "$IDField", "$IDField_TO")  ;
		 }
		 else
		 {
		    String  $IDField_OPERATOR = request.getParameter("$IDField_OPERATOR");
				qStr.addNumberParam("$IDField", $IDField_OPERATOR );
    ...
    // Add  Search Fields 
	 ...
	 // Old Code: qStr.addStringParam("Mobile", false);
	  short Mobile_SEARCHTYPE = RequestHelper.getShort(request, "Mobile_SEARCHTYPE");
	  qStr.addStringParam("Mobile", Mobile_SEARCHTYPE);
	 // For multiple selection select drop list use: qStr.addMultiSelectParam("Mobile", true );
	
	      qStr.addNumberParam("CurrentStatus");
		/*
	   if(request.getParameter("CurrentStatus_RNG_CHK") != null)
		 {
		    qStr.addNumberInRangeParam("CurrentStatus", "CurrentStatus", "CurrentStatus_TO")  ;
		 }
		 else
		 {
		    String  CurrentStatus_OPERATOR = request.getParameter("CurrentStatus_OPERATOR");
				qStr.addNumberParam("CurrentStatus", CurrentStatus_OPERATOR );
		 }
		 */
...
<form action="<%=thisFile %>"  accept-charset="UTF-8" id="$TABLENAME_list" method="post">
...
<table class="table table-bordered table-striped Rslt-Act-tbl" id="$TABLENAME_Result_tbl">
<thead>
<tr>
<th>
<% if(bResultActionCheckbox == true){ %>
<span class="checkbox-custom checkbox-inline checkbox-primary" data-original-title="Check / Uncheck All" data-trigger="hover" data-placement="top" data-toggle="tooltip"><input type="checkbox" name="$TABLENAME_Result_checkall" id="$TABLENAME_Result_checkall"><label></label></span>&nbsp;&nbsp;
<% } %>
</th>
<th>Name</th>
...
<td>
<% if(bResultActionCheckbox == true){ %>
<span class="checkbox-custom checkbox-inline checkbox-primary"><input type="checkbox" name="$IDField" id="$IDField_<%=$BeanName.$IDField %>"  value="<%=$BeanName.$IDField %>"><label></label></span>&nbsp;&nbsp;
<% } %>
<%=sno %> 
<button onclick="NavigateTo('<%=thisFile %>?Action=Show&$IDField=<%=$BeanName.$IDField %><%=ForeignKeyParam %><%=ReturnPathLink %>')" class="btn btn-pure btn-primary icon wb-eye" type="button" style="font-size: 20px;padding: 0;margin-left: 5px;" data-original-title="View" data-trigger="hover" data-placement="top" data-toggle="tooltip"></button>
</td>
	<td><%if($BeanName.Gender.equalsIgnoreCase("Male")){ %><i aria-hidden="true" class="icon fa fa-male iccolor"></i><% }else{ %><i class="icon fa fa-female iccolor" aria-hidden="true"></i><% } %>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="callselect('<%=$BeanName.$IDField %>','<%=ShowItem.show$CTABLENAMEName($BeanName.$IDField, 1) %>')"><%=ShowItem.show$CTABLENAMEName($BeanName.$IDField, 1) %></a></td>
...
<div class="panel" style="margin-bottom: 15px;">
  <div class="panel-heading" style="padding: 0 0 10px;">
    <h3 class="panel-title page-title-action"><%=PageTitle %>
		...
		<% } %>
		<% if(bAllowUpdate == true){ %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;" data-original-title="Edit" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="NavigateTo('<%=thisFile %>?Action=Change&$IDField=<%=$BeanName.$IDField %><%=ForeignKeyParam %><%=ReturnPathLink %>')"><i aria-hidden="true" class="icon wb-pencil" style="margin: 0;"></i></button>
    <% } %>
		</h3>
	</div>
<!-- 
              ...
              <div class="tab-content">
                <div class="tab-pane active animation-slide-left" id="Personal" role="tabpanel">
                    ...
                    <li class="list-group-item">
                      ...
                      <p class="list-group-item-text"><%if($BeanName.Gender.equalsIgnoreCase("Male")){ %><i aria-hidden="true" class="icon fa fa-male fa-lg iccolor"></i><% }else{ %><i class="icon fa fa-female iccolor" aria-hidden="true"></i><% } %>&nbsp;&nbsp;<%=ShowItem.show$CTABLENAMEName($BeanName.$IDField, 1) %></p>
                    </li>
                    <li class="list-group-item">
...
function callselect(id,name)
{
window.parent.closeModal(id,name);
}

// Support for check boxes in data list.


$("#$TABLENAME_Result_checkall").click(function () {
   if ($("#$TABLENAME_Result_checkall").is(':checked')) {
        $("#$TABLENAME_Result_tbl input[type=checkbox]").each(function () {
