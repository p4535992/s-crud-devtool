...
import  javax.sql.* ;	
/*
   Code generated on : 31/05/2016 11:24 PM
...
public void copyTo(com.db.$DATABASE.Accesslogof$TABLENAMEBean other )
{
...
public void copyFrom(com.db.$DATABASE.Accesslogof$TABLENAMEBean other )
{
...
stmt.setTimestamp( 4 , LogoutTime ) ; 
stmt.setString( 5 , IPAddress ) ; 
stmt.setShort( 6 , Flag ) ; 
...
LoginTime = rslt.getTimestamp("LoginTime"); 
LogoutTime = rslt.getTimestamp("LogoutTime"); 
...
throws java.sql.SQLException 
{
 ...
 try{
    String query_add = " INSERT INTO `accesslogof$TABLENAME` ( `RecordID`, `$IDFIELD`, `LoginTime`, `LogoutTime`, `IPAddress`, `Flag` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
 ...
 throws java.sql.SQLException 
{
  ...
  String query_insert = " INSERT INTO `accesslogof$TABLENAME` ( `RecordID`, `$IDFIELD`, `LoginTime`, `LogoutTime`, `IPAddress`, `Flag` ) VALUES ( ?, ?, ?, ?, ?, ?  ) ";
	...
	throws java.sql.SQLException 
{
  ...
  try
	  {
    String query_update = " UPDATE `accesslogof$TABLENAME` SET `RecordID` = ?, `$IDFIELD` = ?, `LoginTime` = ?, `LogoutTime` = ?, `IPAddress` = ?, `Flag` = ?  WHERE `accesslogof$TABLENAME`.`RecordID` = "+update_RecordID+" " ;
...
throws java.sql.SQLException 
{
 ...
 if(conn==null) return false ;
 try{
    String query_del = "  DELETE FROM `accesslogof$TABLENAME` WHERE `accesslogof$TABLENAME`.`RecordID` = "+delete_RecordID+" " ;
...
throws java.sql.SQLException 
{
 ...
 if(conn==null)return false;
 try{
    String query_locate = "  SELECT *  FROM `accesslogof$TABLENAME` WHERE  `accesslogof$TABLENAME`.`RecordID` = "+locate_RecordID+" " ;
...
throws java.sql.SQLException 
{
   ...
   try
	 {
    String query_locate_on_field = "  SELECT * FROM `accesslogof$TABLENAME`  WHERE `"+field+"` = '"+value+"' " ;
...
/* General purpose table opening method  */	
  ...
  throws java.sql.SQLException
  {
      ...
      _query =   "  SELECT * FROM `accesslogof$TABLENAME`  "+WhereClause+" "+OrderByClause ;
...
/* Additional table opening method as MySQL supports pagination through limit and offset key words */	
  ...
  throws java.sql.SQLException
  {
      _conn = Connect();
	   if(_conn==null) return null;
     _query =   "  SELECT * FROM `accesslogof$TABLENAME`  "+WhereClause+" "+OrderByClause+" LIMIT "+nLimit+" OFFSET "+nOffSet+" " ;
     _stmt = _conn.createStatement();
     _result  = _stmt.executeQuery(_query); 
     return _result ;
  } // end method openTable();
...
throws java.sql.SQLException 
{
 ...
 java.sql.Connection conn = Connect();
 if(conn==null) return 0;
 String query_count = "  SELECT COUNT(*) FROM `accesslogof$TABLENAME`  "+WhereClause;
    try
    {
...
} /* End of class declaration for Accesslogof$TABLENAMEBean */	
