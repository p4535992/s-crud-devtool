...
<%! 
boolean CheckDuplicateEntry($BeanPackage.Accesslogof$TABLENAMEBean dbBean )
{
 /* 
...
final String QUERY_OBJECT_ID = "accesslogof$TABLENAMEQuery" ;
...
if(nAction==ADD_RECORD)
{
     ...
     Alof$BeanName.RecordID= 0;

/* Auto increment column: 
     try  
      {
           Alof$BeanName.RecordID = Integer.parseInt(request.getParameter("RecordID")) ;
      } catch( NumberFormatException ex)
     { 
         Alof$BeanName.RecordID = 0 ;
     }
*/
     try  
      {
           Alof$BeanName.$IDField = Integer.parseInt(request.getParameter("$IDField")) ;
      } catch( NumberFormatException ex)
     { 
         Alof$BeanName.$IDField = 0 ;
     }
    Alof$BeanName.LoginTime = DateTimeHelper.requestDateTimePicker( request, "LoginTime" ); // new java.sql.Timestamp(System.currentTimeMillis()); 
     Alof$BeanName.LogoutTime = DateTimeHelper.requestDateTimePicker( request, "LogoutTime" ); // new java.sql.Timestamp(System.currentTimeMillis()); 
      Alof$BeanName.IPAddress = request.getParameter("IPAddress");
    try 
     { 
          Alof$BeanName.Flag = Short.parseShort(request.getParameter("Flag")) ;
    } catch( NumberFormatException ex)
     {     
         Alof$BeanName.Flag = 0 ;
    }


		if( bAllowAdd == true )
		{
		    if(CheckDuplicateEntry( Alof$BeanName ))
        {
           Alof$BeanName.addRecord();
           MessageText = "One record added. New ID = "+Alof$BeanName._autonumber ;
...
{
         Alof$BeanName.locateRecord(nRecordID);
    //  $CHECK Alof$BeanName.RecordID= nRecordID ;
    /* Auto increment column: 
     try  
      {
           Alof$BeanName.RecordID = Integer.parseInt(request.getParameter("RecordID")) ;
    ...
    try 
     ...
     {     
	  ...
	  if(bAllowUpdate == true)
		{
	      Alof$BeanName.updateRecord(nRecordID);
...
if(nAction==DELETE_RECORD )
{
    ...
    // $CHECK
		...
		{
       Alof$BeanName.deleteRecord(nRecordID);
        ...
        // Alof$BeanName.executeUpdate(DelRef);
 ...
 if(nAction == PROCESS_CHECKED)
 {
    qStr  = new com.webapp.utils.MakeQueryString( request, application );
	  ...
	  String ChkQuery = qStr.SQL( " SELECT * FROM  `accesslogof$TABLENAME` "+ WhrCls + " ORDER BY `RecordID` " );
		...
		if("Delete".equalsIgnoreCase(CheckedAction) && chk_count > 0  )
		{
		    /* Poll through records to be deleted in case values are there in dependent table 
				   ...
				   Alof$BeanName.openTable(WhrCls, " ORDER BY `RecordID` ");
           while(Alof$BeanName.nextRow())
           {
				...
				*/
				if(bAllowDelete == true)
				{
				 int del_cnt = Alof$BeanName.executeUpdate( qStr.SQL( "DELETE FROM `accesslogof$TABLENAME` "+WhrCls+" ") );
		...
		if("Activity".equalsIgnoreCase(CheckedAction) && chk_count > 0 )
		{
          ...
          String redirect  = "accesslogof$TABLENAME-selection-activity.jsp?Count="+chk_count+"&WhereClause="+WhereParam+"&ReturnPath="+ReturnPath; 
...
<head>
  <jsp:include page ="/include-page/common/meta-tag.jsp" flush="true" />
	<title>Accesslog of $CTABLENAME</title>
...
<body class="<%=menuTypeClass %> <%=SiMngrBn.LoginRole %>" >
  ...
  <div class="page animsition">
    <div class="page-content container-fluid">
  	  <div class="row">
        <div class="col-sm-6">
  			  <h3 class="page-title-res"><i aria-hidden="true" class="icon fa fa-cogs iccolor"></i>Accesslog of $CTABLENAME</h3>
  	    </div>
        <div class="col-sm-6">
          <ol class="breadcrumb breadcrumb-res">
            ...
            <li class="active">Accesslog of $CTABLENAME</li>
          </ol>			
  	    </div>
  		</div>	
			<hr class="hr-res" />
...
%>			
<form action="<%=thisFile %>" method="POST"  accept-charset="UTF-8" name="accesslogof$TABLENAME_Add" id="accesslogof$TABLENAME_Add"   >
...
<div class="row">
    <div class="form-group col-sm-6">
	     <label for="$IDField" class="control-label blue-grey-600" >$IDField</label>
...
Alof$BeanName.locateRecord(nRecordID);
%>
<form action="<%=thisFile %>" method="POST"  accept-charset="UTF-8" name="accesslogof$TABLENAME_Update" id="accesslogof$TABLENAME_Update"    >
...
<div class="row">
    <div class="form-group col-sm-6">
	     <label for="$IDField" class="control-label blue-grey-600" >$IDField</label>
       <input type="text" name="$IDField" id="$IDField" class="form-control" value="<%=Alof$BeanName.$IDField %>"  />
	  </div>
 
    <div class="form-group col-sm-6">
	    <label for="LoginTime" class="control-label blue-grey-600" >LoginTime</label>
		  <input type="text" name="LoginTime" id="LoginTime" class="form-control" value="<%=DateTimeHelper.showDateTimePicker(Alof$BeanName.LoginTime) %>"  />
		</div>
</div>
<div class="row" >
    ...
    <div class="form-group col-sm-6">
	    <label for="LogoutTime" class="control-label blue-grey-600" >LogoutTime</label>
		  <input type="text" name="LogoutTime" id="LogoutTime" class="form-control" value="<%=DateTimeHelper.showDateTimePicker(Alof$BeanName.LogoutTime) %>"  />
		</div>

    <div class="form-group col-sm-6">
	     <label for="IPAddress" class="control-label blue-grey-600" >IPAddress</label>
       <input type="text" name="IPAddress" id="IPAddress" class="form-control" value="<%=Alof$BeanName.IPAddress %>"  />
	  </div>
</div>
<div class="row" >
    ...
    <div class="form-group col-sm-6">
	     <label for="Flag" class="control-label blue-grey-600" >Flag</label>
       <input type="text" name="Flag" id="Flag" class="form-control" value="<%=Alof$BeanName.Flag %>"  />
	  </div>
<div class="form-group col-sm-6">&nbsp;</div><div class="form-group col-sm-6">&nbsp;</div>
</div> <!-- End div class=row -->
...
%>
<form action="<%=thisFile %>" method="POST"  accept-charset="UTF-8" class="form-horizontal" id="accesslogof$TABLENAME_Search">
...
<div class="panel panel-form-box panel-form-box-primary">
  <div class="panel-heading panel-form-box-header panel-form-box-with-border">
    <h3 class="panel-title page-title-action"><%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="submit" style="font-size: 10px;margin-left: 5px;" data-original-title="Go To Result Page" data-placement="bottom" data-toggle="tooltip" data-trigger="hover"><i aria-hidden="true" class="icon wb-arrow-right" style="margin: 0;"></i></button>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="reset" style="font-size: 10px;margin-top: -5px;" data-original-title="Reset Form" data-placement="bottom" data-toggle="tooltip" data-trigger="hover"><i aria-hidden="true" class="icon wb-refresh" style="margin: 0;"></i></button>
    </h3>
	</div>
...
<div class="panel-body panel-form-box-body container-fluid">
      ...
      <div class="form-group">
        <label class="col-sm-3 control-label blue-grey-600">$CTABLENAME :</label>
				<div class="col-sm-4">
          <div class="input-group">
            ...
            <span class="input-group-btn">
						   <button class="btn btn-default btn-outline" tabindex="-1" type="button" onclick="{ $('#AdminName').val('');$('#$IDField').val(''); }"><i aria-hidden="true" class="icon fa fa-refresh"></i></button>
...
<!-- 
      <div class="form-group">
        <label class="control-label blue-grey-600 col-sm-3">$IDField :
          <span class="checkbox-custom checkbox-inline checkbox-primary" style="padding-top: 2px;display:none;">&nbsp;<input type="checkbox" name="AdminID_RNG_CHK" id="AdminID_RNG_CHK" onchange="CheckSearchRange('$IDField')"><label for="AdminID_RNG_CHK"></label></span>
        </label>
 ...
 -->					
      ...
      <div class="form-group">
        <label class="col-sm-3 control-label blue-grey-600">Login Time :</label>
        <div class="col-sm-3">
      ...
      <div class="form-group">
        <label class="col-sm-3 control-label blue-grey-600">Logout Time :</label>
        <div class="col-sm-3">
									<div class="input-group input-group-icon" >
      ...
      <div class="form-group">
        ...
        <div class="col-sm-2">
      	  <select name="OrderBy" id="OrderBy" class="form-control show-tick" data-plugin="selectpicker">
      		  ...
      		  <option value="$IDField" >$IDField</option>
...
else
{
	...
	else // else of  if(QryObj != null )
	{
     ...
     qStr.setTablename(Alof$BeanName._tablename);
    ...
    // Add  Search Fields 
	   ...
	   if(request.getParameter("AdminID_RNG_CHK") != null)
		 {
		    qStr.addNumberInRangeParam("$IDField", "$IDField", "AdminID_TO")  ;
		 }
		 else
		 {
		    String  AdminID_OPERATOR = request.getParameter("AdminID_OPERATOR");
				qStr.addNumberParam("$IDField", AdminID_OPERATOR );
		 }
		 */
 	 // For multiple selection select drop list use: qStr.addMultiSelectParam("$IDField", false );
     ...
     if(dtLogout != null ) qStr.addCriterion(CriteriaLogout);
 		 ...
 		 QryObj.jndidsn  = "jdbc/shreedb" ;
     QryObj.table = "accesslogof$TABLENAME"  ;
     ...
     QryObj.count = Alof$BeanName.recordCount(WhereClause); 
     QryObj.offset = 0 ;
     QryObj.pagesize = 0 ;
		 try
		 {
...
<form action="<%=thisFile %>"  accept-charset="UTF-8" id="accesslogof$TABLENAME_list" method="post">
...
<table class="table table-bordered table-striped Rslt-Act-tbl" id="accesslogof$TABLENAME_Result_tbl">
<thead>
<tr>
<th>
<% if(bResultActionCheckbox == true){ %>
<span class="checkbox-custom checkbox-inline checkbox-primary" data-original-title="Check / Uncheck All" data-trigger="hover" data-placement="top" data-toggle="tooltip"><input type="checkbox" name="accesslogof$TABLENAME_Result_checkall" id="accesslogof$TABLENAME_Result_checkall"><label></label></span>&nbsp;&nbsp;
<% } %>
</th>
<th>Name</th>
<th>Login Time</th>
...
/** Check for pagination support 
  if pagination is supported by SQL implementation ( LIMIT and OFFSET key words ) 
	than call :  Alof$BeanName.openTable(WhereClause, OrderByClause, QryObj.pagesize, QryObj.offset  ); 
	and remove while( Alof$BeanName.skipRow() ){} part of loop.
	This will give much better result that usual openTable(WhereClause, OrderByClause ); method call
	
*/

Alof$BeanName.openTable(WhereClause, OrderByClause );  

int n=0;
while(n< QryObj.offset)
{
 n++ ;
 Alof$BeanName.skipRow();
} 
 n=0;
sno=QryObj.offset;

while(Alof$BeanName.nextRow() && n < QryObj.pagesize )
{
sno++;
n++;
DelWarning = "Really want to Delete RecordID : "+Alof$BeanName.RecordID+" " ;
$BeanName.locateRecord(Alof$BeanName.$IDField);
// $CHECK
%>
<tr>
<td>
<% if(bResultActionCheckbox == true){ %>
<span class="checkbox-custom checkbox-inline checkbox-primary"><input type="checkbox" name="RecordID" id="RecordID_<%=Alof$BeanName.RecordID %>"  value="<%=Alof$BeanName.RecordID %>"><label></label></span>&nbsp;&nbsp;
<% } %>

<%=sno %>
<!--  
<button onclick="NavigateTo('<%=thisFile %>?Action=Show&RecordID=<%=Alof$BeanName.RecordID %><%=ForeignKeyParam %><%=ReturnPathLink %>')" class="btn btn-pure btn-primary icon wb-eye" type="button" style="font-size: 20px;padding: 0;margin-left: 5px;" data-original-title="View" data-trigger="hover" data-placement="top" data-toggle="tooltip"></button>
 -->
</td>
<td><%if($BeanName.Gender.equalsIgnoreCase("Male")){ %><i aria-hidden="true" class="icon fa fa-male fa-lg iccolor"></i><% }else{ %><i class="icon fa fa-female fa-lg iccolor" aria-hidden="true"></i><% } %>&nbsp;&nbsp;<a href="<%=thisFile %>?Action=Show&RecordID=<%=Alof$BeanName.RecordID %><%=ForeignKeyParam %><%=ReturnPathLink %>"><%=ShowItem.show$CTABLENAMEName($BeanName.$IDField, 1) %></a></td>
	 <td valign="top" > <%=DateTimeHelper.showDateTimePicker(Alof$BeanName.LoginTime) %></td> 
	  <td valign="top" > <%=DateTimeHelper.showDateTimePicker(Alof$BeanName.LogoutTime) %></td> 
	 <td><% if(Alof$BeanName.IPAddress.equalsIgnoreCase("[::1]")) { %>localhost<% }else{ %><%=Alof$BeanName.IPAddress %><% } %></td>
	
<% if(bAllowUpdate == false && bAllowDelete == false){ %> <% } else { %>
<td class="text-center">
<% if(bAllowUpdate == true){ %>
<button onclick="NavigateTo('<%=thisFile %>?Action=Change&RecordID=<%=Alof$BeanName.RecordID %><%=ForeignKeyParam %><%=ReturnPathLink %>')" class="btn btn-pure btn-primary icon fa fa-pencil" type="button" style="font-size: 20px;padding: 0;margin-left: 5px;" data-original-title="Edit" data-trigger="hover" data-placement="top" data-toggle="tooltip"></button>
<% } %>
<% if(bAllowDelete == true){ %>
<button onclick="DeleteRecord('<%=Alof$BeanName.RecordID %>', '<%=DelWarning %>', '<%=thisFile %>?Action=Delete<%=ForeignKeyParam %><%=ReturnPathLink %>&RecordID=<%=Alof$BeanName.RecordID %>' )" class="btn btn-pure btn-primary icon fa fa-trash-o" type="button" style="font-size: 20px;padding: 0;margin-left: 12px;" data-original-title="Delete" data-trigger="hover" data-placement="top" data-toggle="tooltip"></button>
<% } %>
</td>
<% } %>
</tr>
...
<div class="panel" style="margin-bottom: 15px;">
  <div class="panel-heading" style="padding: 0 0 10px;">
    <h3 class="panel-title page-title-action"><%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;margin-left: 5px;" data-original-title="cancel" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="history.go(-1);"><i aria-hidden="true" class="icon wb-close" style="margin: 0;"></i></button>
		<% if(bAllowDelete == true){ %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;margin-left: 5px;" data-original-title="Delete" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="DeleteRecord('<%=Alof$BeanName.RecordID %>', '<%=DelWarning %>', '<%=thisFile %>?Action=Delete<%=ForeignKeyParam %><%=ReturnPathLink %>&RecordID=<%=Alof$BeanName.RecordID %>' )"><i aria-hidden="true" class="icon wb-trash" style="margin: 0;"></i></button>
		<% } %>
		<% if(bAllowUpdate == true){ %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;" data-original-title="Edit" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="NavigateTo('<%=thisFile %>?Action=Change&RecordID=<%=Alof$BeanName.RecordID %><%=ForeignKeyParam %><%=ReturnPathLink %>')"><i aria-hidden="true" class="icon wb-pencil" style="margin: 0;"></i></button>
    <% } %>
		</h3>
	</div>
<!-- 
...
{
var options = {
        url: "<%=appPath %>/admin/user_searchpopup/search$TABLENAME-popup.jsp",
        title:'<i class="icon fa fa-cogs iccolor" aria-hidden="true"></i>&nbsp;Select $CTABLENAME',
        size: eModal.size.lg
    };
...
$("#accesslogof$TABLENAME_Result_checkall").click(function () {
   if ($("#accesslogof$TABLENAME_Result_checkall").is(':checked')) {
        $("#accesslogof$TABLENAME_Result_tbl input[type=checkbox]").each(function () {
	   ...
	   var sel_count = ( sel_items !=null ) ?  sel_items.length: 0 ;
        ...
        swal({
          ...
          showCancelButton: true
            ...
            $('#accesslogof$TABLENAME_list' ).submit();
        }, function(dismiss) {
          // dismiss can be 'cancel', 'overlay', 'close', 'timer'
          if (dismiss === 'cancel') {
            //toastr.info("Delete Cancel !");
...
// New Entry Form  
  ...
  $("#accesslogof$TABLENAME_Add").validate(); // Please put class="required" in mandatory fields
...
// Update Entry Form  
  ...
  $("#accesslogof$TABLENAME_Update").validate(); // Please put class="required" in mandatory fields
...
// Search Form  
  ...
  $("#accesslogof$TABLENAME_Search").validate(); // Please put class="required" in mandatory fields
<%
 } 
%>
...
/* Search autocomplete support -Start */
  ...
  <%  
    ...
    serviceUrl: "<%=appPath %>/include-page/master/autocomplete-list.jsp",
		params: {table:"accesslogof$TABLENAME",field:"?FieldName"} 
	  });
    */
