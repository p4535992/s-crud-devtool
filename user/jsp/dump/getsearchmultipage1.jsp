...
<div class="panel">
  <div class="panel-heading">
    <h3 class="panel-title page-title-action"><\%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-left: 5px;" data-original-title="Back" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="history.go(-1);"><i aria-hidden="true" class="icon wb-close" style="margin: 0;"></i></button>
...
<div class="panel">
  <div class="panel-heading">
    <h3 class="panel-title page-title-action"><\%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="submit" style="font-size: 10px;margin-left: 5px;" data-original-title="Go To Result" data-placement="bottom" data-toggle="tooltip" data-trigger="hover"><i aria-hidden="true" class="icon wb-arrow-right" style="margin: 0;"></i></button>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="reset" style="font-size: 10px;" data-original-title="Reset Form" data-placement="bottom" data-toggle="tooltip" data-trigger="hover"><i aria-hidden="true" class="icon wb-refresh" style="margin: 0;"></i></button>
    </h3>
	</div>
   ...
   int nSerachFieldType = rsmd.getColumnType(rslt.findColumn(SearchFields[n])) ;
	switch(nSerachFieldType)
	{
	  ...
	  case java.sql.Types.BIGINT:
		%>
      <div class="form-group">
        <label for="<%=SearchFields[n] %>" class="col-sm-3 control-label blue-grey-600"><%out.print("<"+"%=FieldLabel["+BeanName+"_"+SearchFields[n]+"] %"+">" ) ;%> :</label>
        <div class="col-sm-2"><input type="text" name="<%=SearchFields[n] %>" id="<%=SearchFields[n] %>" class="form-control" /></div>
      </div>	
<!-- 
      <div class="form-group">
        <label class="control-label blue-grey-600 col-sm-3"><%=SearchFields[n] %> :
      	...
      	<span class="visible-xs-*">&nbsp;</span>
      	<div class="col-sm-2"><input type="text" name="<%= SearchFields[n] %>" id="<%= SearchFields[n] %>" class="form-control" /></div>
      	<label class="control-label blue-grey-600 col-sm-1 text-center" id="<%= SearchFields[n] %>_RNG_LBL" style="display:none;width: 5.333%;margin-bottom: 0px;">TO</label>
...
<!-- 
 ...
 -->
      <div class="form-group">
        <label class="col-sm-3 control-label blue-grey-600"><%out.print("<"+"%=FieldLabel["+BeanName+"_"+SearchFields[n]+"] %"+">" ) ;%> :</label>
	 ...
	 %>
      <div class="form-group">
        <label class="col-sm-3 control-label blue-grey-600"><%out.print("<"+"%=FieldLabel["+BeanName+"_"+SearchFields[n]+"] %"+">" ) ;%> :
        <span class="checkbox-custom checkbox-inline checkbox-primary" style="padding-top: 2px;display:none;"><input type="checkbox" name="<%= SearchFields[n] %>_RNG_CHK" id="<%= SearchFields[n] %>_RNG_CHK" onchange="CheckSearchRange('<%= SearchFields[n] %>')"><label for="<%= SearchFields[n] %>_RNG_CHK"></label></span>
      	</label>
      	<div class="col-sm-1" id="<%=SearchFields[n] %>_OPR_BLK"><\%=NumberSearchdroplist.getOperatorList("<%=SearchFields[n] %>_OPERATOR", "<%=SearchFields[n] %>_OPERATOR", "form-control show-tick", "data-plugin='selectpicker'" ) %></div>
      	<span class="visible-xs-*">&nbsp;</span>
...
<table class="table table-bordered table-striped Rslt-Act-tbl" id="<%=TableName%>_Result_tbl">
<thead>
<tr>
<th><% if(bCheckBox){ %>
<\% if(bResultActionCheckbox == true){ %>
<span class="checkbox-custom checkbox-inline checkbox-primary" data-original-title="Check / Uncheck All" data-trigger="hover" data-placement="top" data-toggle="tooltip"><input type="checkbox" name="<%=TableName%>_Result_checkall" id="<%=TableName%>_Result_checkall"><label></label></span>&nbsp;&nbsp;
<\% } %><% } %>
</th>
<% 
...
<div class="panel">
  <div class="panel-heading">
    <h3 class="panel-title page-title-action"><\%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-left: 5px;" data-original-title="Back To Result" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="NavigateTo('<\%=thisFile %>?<\%=DEFAULT_ACTION %><\%=ForeignKeyParam %><\%=ReturnPathLink %>')" ><i aria-hidden="true" class="icon wb-close" style="margin: 0;"></i></button>
		<\% if(bAllowDelete == true){ %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-left: 5px;" data-original-title="Delete" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="AppConfirm('question','Are you sure ?','<\%=thisFile %>?Action=Delete<\%=ForeignKeyParam %><\%=ReturnPathLink %>&<%=IDField  %>=<\%=<%=BeanName  %>.<%=IDField  %> %>','<\%=DelWarning %>')" ><i aria-hidden="true" class="icon wb-trash" style="margin: 0;"></i></button>
		<\% } %>
		<\% if(bAllowUpdate == true){ %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;" data-original-title="Edit" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="NavigateTo('<\%=thisFile %>?Action=Change&<%=IDField %>=<\%=<%=BeanName  %>.<%=IDField  %> %><\%=ForeignKeyParam %><\%=ReturnPathLink %>')"><i aria-hidden="true" class="icon wb-pencil" style="margin: 0;"></i></button>
    <\% } %>
		</h3>
	</div>
