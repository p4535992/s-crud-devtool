...
<td width="50%" align="right"><a href="<\%=thisFile %>?Action=New<\%=ForeignKeyParam %><\%=ReturnPathLink %>"><b>Create new record</b></a></td>
</tr>
</table>
 -->
...
<tr>
   <td><span class="label"><%=SearchFields[n] %>:</span></td>
	 <td><span class="dataitem"><%  out.print("<dtag:DatePicker  CalendarImage=\"<"+"%=CalendarImage %"+">\" ElementName=\""+SearchFields[n]+"\" ElementID=\""+SearchFields[n]+"\"  />"); %></span> <!-- Set  year range as tag attribute if relevent:-  YearRange="1900:2100"    --></td> 
</tr>
 <%
 }
 else if(rsmd.getColumnType(rslt.findColumn(SearchFields[n]))==java.sql.Types.TIMESTAMP)
 {
 %>
 <tr>
   <td><span class="label"><%=SearchFields[n] %>:</span></td>
...
<br/><a href="selection-activity.jsp?Count=<\%=QryObj.count %>&WhereClause=<\%=WhrParam %>&ReturnPath=<\%=RetPath %>" <% if( bAltTxt ){ %> onmouseover="writetxt('sel_menu_help')" onmouseout="writetxt(0)" <% } %>  >Selection Activity</a>
</td>
<td width="25%">
