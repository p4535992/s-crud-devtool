...
<a href="<\%=thisFile %>?Action=Search&Activity=<\%=Activity %><\%=ForeignKeyParam %><\%=ReturnPathLink %>">Search Again</a>
<% 
if(bCheckBox)
{ 
%>
...
<tr>
<td width="10%" align="left" ><a href="<\%=thisFile %><\%=QryParam %><\%=ForeignKeyParam %><\%=ReturnPathLink %>&Offset=0"><%=FIRST %></a></td>
<td width="20%" align="left" > 
<\% 
if(bPriv )
{ 
%>
<a href="<\%=thisFile %><\%=QryParam %><\%=ForeignKeyParam %><\%=ReturnPathLink %>&Offset=<\%=(nPrevOffset) %>"><%=PREVIOUS %> </a>[<\%=nPrevOffset+1 %> to <\%=nPrevOffset+nRows %> ]
<\% 
}
else
{ 
...
{ 
%>
<a href="<\%=thisFile %><\%=QryParam %><\%=ForeignKeyParam %><\%=ReturnPathLink %>&Offset=<\%=nNextOffset %>"><%=NEXT %></a> [<\%=nNextOffset+1 %> to <\%=  nNextOffset+nRows %> ]
...
<a href="<\%=thisFile %><\%=QryParam %><\%=ForeignKeyParam %><\%=ReturnPathLink %>&Offset=<\%=LastBlock %>"><%=LAST %></a>
</td>
</tr>
</table>
...
<script type="text/javascript">
<!--
  goto_block_url =  "<\%=thisFile %><\%=QryParam %><\%=ForeignKeyParam %><\%=ReturnPathLink %>"
// -->
</script>
<\% 
} // end if(nCount>nRows)
