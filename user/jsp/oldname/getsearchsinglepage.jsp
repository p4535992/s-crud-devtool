...
<tr>
     <td><span class="label">Search By ID:</span></td>
         ...
         <span class="dataitem">
          ...
          <input type="text" name="<%=IDField  %>" id="<%=IDField  %>" size="10" />
...
{ 
         ...
         <td>
            <span class="dataitem">
				     <!-- Num. Operator --><span id="<%= SearchFields[n] %>_OPR_BLK"><\%=NumberSearch.getOperatorList("<%= SearchFields[n] %>_OPERATOR", "<%= SearchFields[n] %>_OPERATOR", null )  %> </span>
 					   <!-- Field Input --><input type="text" name="<%=SearchFields[n] %>"  id="<%=SearchFields[n] %>" size="10"   />
    ...
    %>
     <tr>
        ...
        <td>
           ...
           <!-- Field Input --><input type="text" name="<%=SearchFields[n] %>"  id="<%=SearchFields[n] %>"  />
           </span>
        </td>
     </tr>
 ...
 <%	 
  ...
  <tr>
       <td><span class="label"><%= SearchFields[n] %>:</span></td>
       <td><span class="dataitem"><input type="text" name="<%=SearchFields[n] %>" id="<%=SearchFields[n] %>"  /></span></td>
  </tr>
<%	 
	 break;
...
<\% if(bAllowAdd  == true){ %> <button type="button" name="NewRecordButton" onclick='NavigateTo("<\%=thisFile %>?Action=New<\%=ForeignKeyParam %><\%=ReturnPathLink %>")' >Create New <%=EntityName  %>'s Record</button> <\% } %>
</div>
...
<br/><a href="<%=TableName %>-selection-activity.jsp?Count=<\%=ResultCount %>&WhereClause=<\%=WhrParam %>&ReturnPath=<\%=RetPath %>" <% if( bAltTxt ){ %> onmouseover="writetxt('sel_menu_help')" onmouseout="writetxt(0)" <% } %>  >Selection Activity</a>

</td>
<td width="25%" align="center"> <\% if(bAllowAdd  == true){ %> <a href="<\%=thisFile %>?Action=New<\%=ForeignKeyParam %><\%=ReturnPathLink %>"><b>Create New <%=EntityName  %>'s Record</b></a> <\% } %> </td>
