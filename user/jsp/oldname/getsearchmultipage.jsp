...
SiMngrBn.locateRecord(LogUsr.AdminID);
<% } %>
<jsp:include page="../forms/formtablefieldlabel.jsp" flush="true"><jsp:param name="JNDIDSN" value="<%=JNDIDSN %>" /><jsp:param name="TableName" value="<%=TableName %>" /><jsp:param name="BeanName" value="<%=BeanName %>" /></jsp:include>
   final int DEFAULT = 0 ;
...
// Show data flag
<%=WebApp %>.appMakeQueryString qStr =  null ;
...
<div class="panel panel-form-box panel-form-box-primary">
  <div class="panel-heading panel-form-box-header panel-form-box-with-border">
    <h3 class="panel-title page-title-action"><\%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;margin-left: 5px;" data-original-title="Back" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="history.go(-1);"><i aria-hidden="true" class="icon wb-close" style="margin: 0;"></i></button>
...
<div class="panel panel-form-box panel-form-box-primary">
  <div class="panel-heading panel-form-box-header panel-form-box-with-border">
    <h3 class="panel-title page-title-action"><\%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="submit" style="font-size: 10px;margin-top: -5px;margin-left: 5px;" data-original-title="Go To Result" data-placement="bottom" data-toggle="tooltip" data-trigger="hover"><i aria-hidden="true" class="icon wb-arrow-right" style="margin: 0;"></i></button>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="reset" style="font-size: 10px;margin-top: -5px;" data-original-title="Reset Form" data-placement="bottom" data-toggle="tooltip" data-trigger="hover"><i aria-hidden="true" class="icon wb-refresh" style="margin: 0;"></i></button>
    </h3>
	</div>
...
<!--  
    ...
    <label for="<%=SearchFields[n] %>" class="col-md-3 col-sm-4 col-xs-12 control-label blue-grey-600"><%out.print("<"+"%=FieldLabel["+BeanName+"_"+SearchFields[n]+"] %"+">" ) ;%> :</label>
      <div class="input-group input-group-icon" >
        <input type='text' name="<%=SearchFields[n] %>" id="<%=SearchFields[n] %>" class="form-control datetimepicker readonlybg" >
...
<% } %><thead>
<tr>
<th<% if(bDatatablesJSCSS){%> class="wd_col_visible"<% } %>><% if(bCheckBox){ %>
<\% if(bResultActionCheckbox == true){ %>
<span class="checkbox-custom checkbox-inline checkbox-primary" data-original-title="Check / Uncheck All" data-trigger="hover" data-placement="right" data-toggle="tooltip"><input type="checkbox" name="<%=TableName%>_Result_checkall" id="<%=TableName%>_Result_checkall"><label></label></span>&nbsp;&nbsp;
<\% } %><% } %>&nbsp;
</th>
<% 
for (n=0 ;ShowFields!=null && n< ShowFields.length ; n++)
{ %><th<% if(bDatatablesJSCSS){%> class="wd_col_visible"<% } %>><%out.print("<"+"%=FieldLabel["+BeanName+"_"+ShowFields[n]+"] %"+">" ) ;%></th>
<% 
} 
%>
<\% if(bAllowUpdate == false && bAllowDelete == false){ %> <\% } else { %>
<th<% if(bDatatablesJSCSS){%> class="wd_col_visible text-center"<% }else{ %> class="text-center"<% } %>>
...
<\% if(bAllowDelete == true){ %>
<button onclick="DeleteAllChecked()" class="btn btn-icon btn-default btn-outline" type="button" style="padding: 5px;color: #f96868;" data-original-title="Delete All Checked" data-trigger="hover" data-placement="left" data-toggle="tooltip"><i aria-hidden="true" class="icon wb-trash"></i></button>
<\% } %>&nbsp;
</th>
<\% } %>
</tr>
...
<div class="panel panel-form-box panel-form-box-primary">
  <div class="panel-heading panel-form-box-header panel-form-box-with-border">
    <h3 class="panel-title page-title-action"><\%=PageTitle %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;margin-left: 5px;" data-original-title="Back To Result" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="NavigateTo('<\%=thisFile %>?<\%=DEFAULT_ACTION %><\%=ForeignKeyParam %><\%=ReturnPathLink %>')" ><i aria-hidden="true" class="icon wb-close" style="margin: 0;"></i></button>
		<\% if(bAllowDelete == true){ %>
		<button class="btn btn-icon btn-dark btn-outline btn-round btn-sm pull-right" type="button" style="font-size: 10px;margin-top: -5px;margin-left: 5px;" data-original-title="Delete" data-placement="bottom" data-toggle="tooltip" data-trigger="hover" onclick="AppConfirm('question','Are you sure ?','<\%=thisFile %>?Action=Delete<\%=ForeignKeyParam %><\%=ReturnPathLink %>&<%=IDField  %>=<\%=<%=BeanName  %>.<%=IDField  %> %>','<\%=DelWarning %>')" ><i aria-hidden="true" class="icon wb-trash" style="margin: 0;"></i></button>
		<\% } %>
		<\% if(bAllowUpdate == true){ %>
...
%>
  ...
  <% } %><% out.println("<jsp:include page=\"/include-page/js/jvalidate-js.jsp\" flush=\"true\" />"); %>
<script>
<\% 
if(MessageText!=null)
{ 
...
<jsp:include page="form-validation.jsp" flush="true"><jsp:param name="TableName" value="<%=TableName %>" /><jsp:param name="LoginFolderName" value="<%=LoginFolderName %>" /></jsp:include>
